...
const obstacleColors = [
  { pipe: "#2B6623", pipeBorder: "#0B6623" },
  { pipe: "#708238", pipeBorder: "#408238" },
  { pipe: "#3F704D", pipeBorder: "#1f704d" },
  { pipe: "#29AB87", pipeBorder: "#09ab87" },
];
...
class Bird {
  ...
  moveBirdAhead() {
    document.getElementById("bird").style = `width: ${this.width}px; height: ${
      this.height
    }px; background: ${this.color}; transform: translate(${
      this.posX + this.velX
    }px, ${this.posY + this.velY}px);`;
  ...
  collisionDetect() {
    for (let i = 0; i < obstacles.length; ++i) {
      ...
      if (
        this.posX + this.width > obstacle.x &&
        this.posX <= obstacle.x + obstacle.width
      ) {
        if (
          obstacle.type === "top" &&
          this.posY + this.height < obstacle.height
        )
          return true;
        else if (obstacle.type === "bottom" && this.posY > obstacle.y)
          return true;
      }
    }
    return false;
...
window.addEventListener("load", (e) => {
  ...
  for (let i = 0; i < 30; ++i) {
    ...
    const cloud = new Cloud(
      100,
      50,
      random(distance - 60, distance + 60),
      random(0, maxHeight / 2)
    );
    cloud.createCloud();
  }
